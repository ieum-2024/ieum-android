...
class DeleteInterest @Inject constructor(
    private val interestRepository: InterestRepository
){
    operator fun invoke(interestId: Long): Flow<Resource<String>> = flow {
        try {
            emit(Resource.Loading())
            val response = interestRepository.deleteInterest(interestId)
            val body = response.body<ApiUtils.ApiResult<String>>()
            val successMessage = body.response ?: "성공적으로 관심사를 추가했습니다."
            val errorMessage = body.error?.message ?: "관심사 추가에 실패하였습니다."
